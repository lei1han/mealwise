...
三餐教练（英文名 MealWise）：微信小程序减肥教练，人格"亦师亦友 + 一点毒舌"，通过聊天帮助"屡减屡败、有基础但缺监督"的人群靠谱瘦下来。MVP 为纯对话小程序，后端记录关键记忆点。
...
- 已完成迁移：
  ...
  - 板块1 前端设计稿 HTML 原型 → `deliverables/frontend/mealwise-frontend-design/`（2026-08-28，板块 1 会话已收尾；工具产物 `runtime-*.json` 保留为历史快照，其硬编码绝对路径不再用于修复流程）。
- 待迁移：无。
