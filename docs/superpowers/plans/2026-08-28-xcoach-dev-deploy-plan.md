...
- 版本：v1.1（现状 §1 同步至 2026-09-14；工程目录改为 `MealWise/`）
- 日期：2026-09-14
- 依赖：总计划契约 A/B/C、[产品决策记录](2026-09-14-xcoach-product-decisions.md)、工程 [`MealWise/`](../../../MealWise/)、内核快照 `deliverables/backend/`
...
- 目标：**MVP 首发**：Mock/云接入验收 → 真 LLM → 资料完善动线 → 调优专题 → 提审上线。**非 MVP**：订阅消息、定时督促（阶段 4/5，后续版本）。
- 边界：只做「开发/调试/部署」层面；契约改动须回写总计划 §1 与决策记录。
- 前提：微信开发者工具已安装；AppID 以 [`MealWise/project.config.json`](../../../MealWise/project.config.json) 为准（`wxd6def00245936b4c`）。

## 1. 现状盘点（2026-09-14）

| # | 项 | 状态 | 备注 |
|---|---|---|---|
| 1 | **action 对齐** | ✅ 已对齐 | 契约 C 所列 action 已在 `MealWise/cloudfunctions/api/src` 实现；前端 `REAL_ACTIONS` 全量白名单 |
| 2 | **模块格式** | ✅ | `api/index.js` CJS 适配 + `src/` ESM |
| 3 | **持久化** | ✅ | `CloudDB` + `app_config`；`db.ensure` 幂等建表 |
| 4 | **cloudfunctionRoot** | ✅ | `MealWise/` 工程内 `cloudfunctions/api` |
| 5 | **云环境 ID** | ✅ | `cloud1-d6gmjs12rfd5c3925` |
| 6 | **DeepSeek key** | ✅（文档记载已配） | 云函数环境变量 `DEEPSEEK_API_KEY` |
| 7 | **资料完善动线** | 🟡 待开发 | 产品决策 5B：登录后昵称/头像完善再进聊天 |
| 8 | **E2E / USE_MOCK** | 🟡 待定 | 模拟器走查；`USE_MOCK` 全局开关待定 |
| 9 | **调优报告 gap** | 🟡 专题挂起 | 提示词/食物库注入；提审前需专题结论 |
| 10 | **订阅 template_key** | ⏸️ 非 MVP | 阶段 4；模板 ID 申请后接入 |
| 11 | **定时触发器** | ⏸️ 非 MVP | `scheduler.nudge` 逻辑在 api 内核；`config.json` 无 cron |

### 1.1 历史待办（v1.0，已销项）

2026-08-28 初版 §1 中「仅 3 action / MemoryDB / 未配 env」等条目已由 2026-08-31～09-01 工程迁移完成，**仅作考古**，不再作为开工依据。

## 1.2 仍须跟进的部署项

见上表 #7～#9；M5 相关见 §3 阶段 4/5「后续版本」。
...
目标：在开发者工具模拟器里把完整用户旅程（闪屏→授权→**资料完善（待开发）**→摸底定标→体质录入→每日聊天→Sheet 子流程）全部走通。

- **开发者**：在 **`MealWise/`** 工程走查；历史 `mealwise-miniapp/DEV_GUIDE.md` 仅作参考。
...
### 阶段 4：订阅消息（**后续版本 · 非 MVP 首发**，开发者申请 + agent 接入）
...
### 阶段 5：定时督促（**后续版本 · 非 MVP 首发**，agent 配置 + 开发者验证）
